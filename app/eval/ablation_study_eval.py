...
def token_accept_with_length_test(model, dataloader, device, batche_size, lengths, max_seq_length, random_slice, model_name, output_file_path):
    ...
    for L in lengths:
        ...
        accept_rate = eval_accept_rate_for_length(
            model=model,
            dataloader=dataloader,
            prefix_len=L,
            device=device,
...
def main(
    ...
    output_file_path = create_output_file_path(output_dir, 'ablation_study')
    ...
    for item in Path(ckpt_dir).iterdir():
        ...
        if target_dir != '' and target_dir != str(item):
            continue

        model, tokenizer = load_model(item, params, tokenizer_path, max_seq_len, per_device_train_batch_size)
