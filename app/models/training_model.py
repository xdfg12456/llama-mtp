...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.n_local_heads = args.n_heads  # no tensor model parallel
        self.n_local_kv_heads = self.n_kv_heads
        self.n_rep = self.n_local_heads // self.n_local_kv_heads
        self.head_dim = args.dim // args.n_heads

        # Standard (non-model-parallel) projections
...
class Transformer(nn.Module):
    ...
    def forward_trunk(self, tokens: torch.Tensor):
        ...
        return h_trunk, freqs_cis, mask

    def forward_head_from_trunk(
        self,
        ...
        if head_index == 0:
            ...
            layer = self.extra_heads[head_index - 1]

        h = layer(h_trunk, freqs_cis, mask)
    ...
    def forward(self, tokens: torch.Tensor, head_index: int):
        h_trunk, freqs_cis, mask = self.forward_trunk(tokens)
        logits, h = self.forward_head_from_trunk(
            h_trunk=h_trunk,
